# Title: tf_ver2_2 (TF_{en8c})
# memory를 8초 단위로 설정한다.
# Model: with scene context, with 1) traj, 2) state, tf encoder
# Revision Date: 24.06.20
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        # Decoder Hidden
        if pooling_type:
            input_dim = 2 * encoder_h_dim + bottleneck_dim + 512# 64*2+1024=1152
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, 
        ...
        enc_traj = self.encoder(obs_traj_rel) # 수정 obs_traj->obs_traj_rel 240620
        ...
        if self.pooling_type: # 풀링을 한다면
            ...
            mlp_decoder_context_input = torch.cat(
                [
                    ...
                    img_ft.unsqueeze(0).expand(8, -1, -1)     # (8, batch, 512)
                 ]
