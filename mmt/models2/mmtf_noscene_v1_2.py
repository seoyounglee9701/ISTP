...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, last_pos, last_pos_rel, decoder_h, seq_start_end):
        """
        Inputs:
        - last_pos: Tensor of shape (batch, 2) # [133,2]
        - last_pos_rel : Tensor of shape (batch, 2) # [133,2]
        - decoder_h: tensor of shape (batch, decoder_h_dim=128)
        - seq_start_end: A list of tuples which delimit sequences within batch
        Output:
        ...
        # print(f"decoder_h shape #:{decoder_h.shape}") # torch.Size([133, 128])
        ...
        # memory = self.memory_embedding(decoder_h)
        ...
        # print(f"memory shape ###:{memory.shape}") # torch.Size([1, 133, 64])
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, 
        ...
        enc_traj = self.encoder(obs_traj_rel) # 수정 240619
        final_enc_traj = enc_traj[-1] 
        enc_state = self.encoder2(obs_state)
        final_enc_state = enc_state[-1] 
