# Title: tf_ver5
# ver3: 매 타임스텝마다 decoder에서 pooling 하는 부분, decoder embedding = 128 (기존 64)로 설정
# ver2: memory를 8초 단위로 설정한다. 
#
# Model: w/o scene context, with 1) traj, 2) state, tf encoder
# Revision Date: 24.06.08
...
# embedding_dim=embeding_dim이 동작하지 않아서 128로 하드코딩함
class TransformerDecoder(nn.Module):
    """Decoder is part of TrajectoryGenerator"""
    def __init__(
        self, pred_len, 
        embedding_dim=128,#64, # 128,# =64, 
        ...
        self.spatial_embedding = nn.Linear(2, 128)# nn.Linear(2, self.embedding_dim)
    ...
    def forward(self, last_pos, last_pos_rel, memory, seq_start_end):
        ...
        for i in range(self.pred_len): 
            ...
            # print(f"output shape:{output.shape}") # torch.Size([1, 133, 64])
            ...
            #     print(f"predicted_trajectory[7].shape:{predicted_trajectory[7].shape}") # torch.Size([1, 133, 64])
            #     break

            if self.pool_every_timestep: # 매 타임스텝별 decoder 정보 취합 (풀링)
                decoder_h = output[-1, :, :] # 마지막 시간 단계에 해당하는 출력을 선택, 모든 배치에 대한 출력을 선택 -> (batch, embedding_dim) -> 배치에 대한 마지막 시간 단계의 디코더 출력을 나타내는 2D 텐서
                # print(f"decoder_h shape:{decoder_h.shape}") # [133,64]

                pool_h = self.pool_net(decoder_h, seq_start_end, curr_pos) 

                decoder_h = torch.cat([decoder_h, pool_h], dim=1)   # 풀링된 정보가 추가된 텐서
                decoder_h = self.mlp(decoder_h)                     # 텐서의 선형 변환과 활성화 함수 적용
                decoder_h = torch.unsqueeze(decoder_h, 0)           # 차원을 늘림, 배치 차원을 추가하여 decoder_h를 디코더 다음 시간 단계 입력으로 사용할 수 있게 함  (1, batch, embedding_dim)
                memory = torch.cat([memory, decoder_h], dim=0)
            
            # next_step = output[-1, :, :]  # 마지막 타임 스텝의 예측 값을 추출
            # print(f"next_step shape:{next_step.shape}") # 
            # predicted_trajectory.append(next_step.unsqueeze(0))
