        ...
        for file_path in all_files:
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                ...
                for _, agent_id in enumerate(agents_in_curr_seq):           # 현재 시퀀스에 있는 각 에이전트 
                    ...
                    # print(f"Agent ID: {agent_id}, curr_agent_seq shape: {curr_agent_seq.shape}")
                    # if(curr_agent_seq.shape[0]<16):
                    #     print(curr_agent_seq)
                    #     sys.exit()
                    # 출력하기
                    # print(curr_agent_seq.shape)
                    # 상태 데이터 필터링: 상태 데이터를 curr_seq_sel_data에서 에이전트 ID로 필터링하여 추출
     
                    if curr_seq_sel_data.shape[1] == 1: # 열의 수
                        # 첫 번째 열 기준으로 필터링
                    ...
                    ## 배열의 형태 확인
                    # print(curr_seq_sel_data.shape) # ex: (235, 1)
                    # ## 배열의 차원 확인
                    ...
                    if check_for_duplicate_data(curr_seq_data, agent_id):
                        ...
                        # curr_agent_seq = np.unique(curr_agent_seq, axis=0)
                        # curr_agent_seq_sel = np.unique(curr_agent_seq_sel, axis=0)
